...
target_image = np.float32(cv2.cvtColor(cv2.imread(os.path.join(os.path.dirname(__file__),'target.jpg')), cv2.COLOR_BGR2RGB)) / 255.0
...
def generate_tme(datapath, save_dir, file_pattern='*.svs', nfile=0, patch_size=384, patch_stride=192, input_size=384, nClass=2, color_norm=True):
    ...
    model_checkpoint = '/rsrch6/home/trans_mol_path/yuan_lab/TIER2/artemis_lei/codes/TMESegBiopsy_public/tmeseg-infer/model/mit-b3-finetunedBRCA-Artemis'  #final model
    ...
    for im_f in imgs:
        ...
        if not os.path.exists(os.path.join(save_dir_file, img_name + '.png')):
            ...
            outData = model.predict(testData_c, batch_size=16)
            ...
            seg_mask = get_colored_segmentation_image(merge_output, nClass, colors=class_colors_artemis)
